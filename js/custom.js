...
function toggleSection(sectionName) {
	const toToggleOn = Array.from(
		document.querySelectorAll(`.section__textbox--${sectionName}`)
	);
	const toToggleOff = Array.from(
		document.querySelectorAll(
			`.section__textbox:not(.section__textbox--inactive)`
		)
	);
